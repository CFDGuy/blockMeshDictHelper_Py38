...
class mergePatchPairs(object):
    def __init__(self,patch1,patch2):
        self.patch1 = patch1
        self.patch2 = patch2

 #----------- 22.03.2017 - Karsten Hasselmann ---------------
class arc(object):
    def __init__(self,start,end,point):
        self.start = start
        self.end = end
        self.point = point
...
#----------- 21.01.2016 - Karsten Hasselmann ---------------
    def __init__(self, type_, name, faces=[], neighbour='0'):
        """ initialize boundary
        ...
        self.name = name
        self.faces = faces
        self.neighbour = neighbour  #only for cyclic patch
...
#----------- 21.01.2016 - Karsten Hasselmann ---------------
    def format(self, vertices):
        ...
        buf.write('{\n')
        buf.write('    type {};\n'.format(self.type_))
        if self.type_ == 'cyclic':
            buf.write('    neighbourPatch {};\n'.format(self.neighbour))
        buf.write('    faces\n')
        buf.write('    (\n')
        for f in self.faces:
            s = f.format(vertices)
...
class BlockMeshDict(object):
    def __init__(self):
        ...
        self.boundaries = {}
#----------- 08.10.2015 - Karsten Hasselmann ---------------
        self.splines = []  #List with splines at edges
#----------- 21.01.2016 - Karsten Hasselmann ---------------
        self.polyLines = []  #List with polyLines at edges
        self.patchPairs = [] #List with mergePatchPairs
#----------- 22.03.2017 - Karsten Hasselmann ---------------
        self.arcs = [] #List with arcs at edges
        
#----------- 08.10.2015 - Karsten Hasselmann ---------------

    def add_spline(self, start, end, points ):
        ...
        """
        self.splines.append(Spline(start, end, points))
        return 0

#----------- 21.01.2016 - Karsten Hasselmann ---------------

    def add_polyLine(self, start, end, points ):
        """add vertex by coordinate and uniq name
        x y z is coordinates of vertex
        name is uniq name to refer the vertex
        returns Vertex object whici is added.
        """
        self.polyLines.append(polyLine(start, end, points))
        return 0
...
#    return 0
    ...
    def add_patchPair(self, patch1, patch2 ):
        ...
        returns Vertex object whici is added.
        """
        self.patchPairs.append(mergePatchPairs(patch1, patch2))
        return 0

#----------- 22.03.2017 - Karsten Hasselmann ---------------
    def add_arc(self, start, end, point):
        """add vertex by coordinate and uniq name
        x y z is coordinates of vertex
        name is uniq name to refer the vertex
        returns Vertex object whici is added.
        """
        self.arcs.append(arc(start, end, point))
        return 0
#
#class mergePatchPairs(object):
#    def __init__(self,patch1,patch2):
...
#-------------------------------------------------------------
    ...
    def add_hexblock(self, vnames, cells, name, grading=SimpleGrading(1, 1, 1)):
        ...
        return b
#----------- 21.01.2016 - Karsten Hasselmann ---------------
    def add_boundary(self, type_, name, faces=[], neighbour='0'):
...
#-------------------------------------------------------------
    ...
    #----------- 22.03.2017 - Karsten Hasselmann ---------------    
        for s in self.arcs:
            buf.write('    arc '+s.start+' '+s.end+ ' ('+str(s.point[0])+' ' +str(s.point[1])+' ' +str(s.point[2])+')'+ '\n')   
    #----------- 08.10.2015 - Karsten Hasselmann ---------------    
        for s in self.splines:
            buf.write('    spline '+s.start+' '+s.end+ '\n')
            buf.write('    ('+ '\n')
            for p in s.points:
                buf.write('        ('+str(p[0])+' ' +str(p[1])+' ' +str(p[2])+')'+ '\n')
                buf.write('    )'+ '\n')
    #----------- 21.01.2016 - Karsten Hasselmann ---------------
        for s in self.polyLines:
            buf.write('    polyLine '+s.start+' '+s.end+ '\n')
            buf.write('    ('+ '\n')
            for p in s.points:
                buf.write('        ('+str(p[0])+' ' +str(p[1])+' ' +str(p[2])+')'+ '\n')
                buf.write('    )'+ '\n')
...
#----------- 21.01.2016 - Karsten Hasselmann ---------------
    def format_mergepatchpairs_section(self):
        buf = io.StringIO()
        buf.write('mergePatchPairs\n')
        buf.write('(\n')
        for b in self.patchPairs:
            buf.write('    ' + '('+ b.patch1 +' '+ b.patch2 + ')' + '\n')   # b[0] = patch1, b[1] = patch2
        buf.write(');') 
        return buf.getvalue()      
#        return '''\
#mergePatchPairs
